...
const translations: any = {
  ...
  US: {
    ...
    questions: [
      ...
      'Are you satisfied with the prep?',
      'Was the communication appropriate?',
      'Was it distinct from other events?',
      'Were safety measures good?',
      'Are you satisfied overall?',
      'Will this experience be helpful?',
    ],
  },
  JP: {
    title: '国際大会満足度調査',
  ...
  VN: {
    ...
    questions: [
      'Cơ sở vật chất có tốt không?',
      'Công tác chuẩn bị có tốt không?',
      'Tổ chức có phù hợp không?',
      'Có điểm khác biệt không?',
      'An toàn có đảm bảo không?',
      'Bạn có hài lòng không?',
      'Trải nghiệm có ích không?',
    ],
  },
};
...
export default function SurveyPage() {
  ...
  const LOGO_URL =
    'https://gongu.copyright.or.kr/cmmn/file/getImage.do?atchFileId=452e64e94c2b6005805cd68110510e336af993dfdb7415bcd4285d9e22c3cdc6&fileSn=1';

  const handleSubmit = async () => {
    if (!info.gender || !info.age || scores.includes(0))
      return alert('Error: Missing info');
    setLoading(true);
  ...
  if (step === 0)
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 text-black">
        ...
        <h1 className="text-2xl font-black mb-8 tracking-tighter text-center text-slate-900">
          {t.title}
        </h1>
        <div className="w-full max-w-xs space-y-4">
          <select
            ...
            className="w-full p-4 rounded-2xl border-2 bg-white font-bold shadow-lg text-black"
          >
            {countryList.map((c) => (
          ...
          >
            ...
            {t.ages.map((ageText: string, idx: number) => (
          ...
          <button
            ...
            className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black text-lg shadow-xl mt-4"
          >
            {t.labels.start}
          </button>
        </div>
    ...
    return (
      <div className="min-h-screen bg-slate-50 p-4 pb-20 text-black">
        <div className="max-w-md mx-auto bg-white rounded-[2.5rem] p-8 shadow-2xl">
          ...
          <button
            onClick={() => setStep(0)}
            className="mb-6 text-slate-400 font-bold"
          >
            ← {t.labels.back}
          </button>
          <h2 className="text-xl font-black text-slate-800 mb-10 text-center italic underline underline-offset-8">
            {t.title}
          </h2>
          <div className="space-y-12">
            {t.questions.map((q: string, i: number) => (
              <div key={i}>
                <p className="font-bold text-slate-700 mb-5 leading-snug">
                  <span className="text-blue-600 mr-2">Q{i + 3}.</span> {q}
                </p>
                <div className="flex justify-between gap-1">
                  {[1, 2, 3, 4, 5].map((n) => (
                    <button
                      ...
                      className={`flex-1 aspect-square rounded-xl border-2 font-black transition-all ${
                        scores[i] === n
          ...
          <button
            ...
            disabled={loading}
            className="w-full bg-slate-900 text-white py-5 rounded-2xl font-black text-xl mt-16 shadow-2xl active:scale-95 disabled:opacity-50"
          >
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-white p-6 text-center text-black">
      ...
      <button
        ...
        className="text-blue-600 font-black border-b-2 border-blue-600 pb-1"
      >
        {t.labels.restart}
      </button>
    </div>
